...
def do_todays_analysis():
    ...
    output_folder_path = f"analysis/monkeypox polio/{' '.join(data_sources)} {date}"
...
def query_and_save_data(data_file_path, aefi_file_path, data_source):
    ...
    df_aefi = pd.DataFrame(unique_aefi_text_and_date, columns=['symptom', 'tweet_text', 'created_at'])
